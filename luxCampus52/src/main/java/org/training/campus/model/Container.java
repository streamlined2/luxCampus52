...
public class Container<K extends Comparable<K>, E extends Entity<K>> implements Iterable<E> {
	...
	public Optional<E> getById(K id) {
		...
		if (index >= 0)
			return Optional.ofNullable((E) data[index]);
	...
	public void add(E e) {
		...
		if (index >= 0)
			throw new IllegalStateException("can't add element that already exists");
	...
	public E update(E e) {
		...
		if (updateAt < 0)
			throw new NoSuchElementException("no element to update");
	...
	public Iterator<E> iterator() {
		return new Iterator<E>() {
			...
			public E next() {
				if (index >= size)
					throw new NoSuchElementException("check if there are elements left before advancing");
				return (E) (data[index++]);
			}
		};
	}
